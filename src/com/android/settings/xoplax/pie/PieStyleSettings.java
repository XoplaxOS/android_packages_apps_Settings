...
package com.android.settings.xoplax.pie;
...
import com.android.settings.xoplax.util.slim.SeekBarPreference;
...
public class PieStyleSettings extends SettingsPreferenceFragment implements
    ...
    private void resetToDefault() {
        ...
        Settings.XOPLAX.putFloat(getActivity().getContentResolver(),
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        ...
        if (preference == mPieBackgroundAlpha) {
            ...
            Settings.XOPLAX.putFloat(getActivity().getContentResolver(),
                    Settings.XOPLAX.PIE_BACKGROUND_ALPHA,
                    val / 100);
            return true;
            ...
            Settings.XOPLAX.putInt(getActivity().getContentResolver(),
                    Settings.XOPLAX.PIE_BACKGROUND_COLOR, intHex);
            return true;
            ...
            int intHex = ColorPickerPreference.convertToColorInt(hex);
            Settings.XOPLAX.putInt(getActivity().getContentResolver(),
                    Settings.XOPLAX.PIE_TEXT_COLOR, intHex);
            return true;
            ...
            int intHex = ColorPickerPreference.convertToColorInt(hex);
            Settings.XOPLAX.putInt(getActivity().getContentResolver(),
                    Settings.XOPLAX.PIE_SNAP_COLOR, intHex);
            return true;
        } else if (preference == mShowText) {
            Settings.XOPLAX.putInt(getContentResolver(),
                    Settings.XOPLAX.PIE_SHOW_TEXT, (Boolean) newValue ? 1 : 0);
            return true;
        } else if (preference == mShowBackground) {
            Settings.XOPLAX.putInt(getContentResolver(),
                    Settings.XOPLAX.PIE_SHOW_BACKGROUND, (Boolean) newValue ? 1 : 0);
            return true;
        } else if (preference == mShowSnap) {
            Settings.XOPLAX.putInt(getContentResolver(),
                    Settings.XOPLAX.PIE_SHOW_SNAP, (Boolean) newValue ? 1 : 0);
            return true;
        } else if (preference == mMirrorRightPie) {
            Settings.XOPLAX.putInt(getContentResolver(),
                    Settings.XOPLAX.PIE_MIRROR_RIGHT,
    ...
    private void updateStyleValues() {
        ...
        intColor = Settings.XOPLAX.getInt(getActivity().getContentResolver(),
                    Settings.XOPLAX.PIE_BACKGROUND_COLOR, -2);
        if (intColor == -2) {
        ...
        mShowSnap.setChecked(Settings.XOPLAX.getInt(getContentResolver(),
                Settings.XOPLAX.PIE_SHOW_SNAP, 1) == 1);

        mShowText.setChecked(Settings.XOPLAX.getInt(getContentResolver(),
                Settings.XOPLAX.PIE_SHOW_TEXT, 1) == 1);

        mShowBackground.setChecked(Settings.XOPLAX.getInt(getContentResolver(),
                Settings.XOPLAX.PIE_SHOW_BACKGROUND, 1) == 1);

        mMirrorRightPie.setChecked(Settings.XOPLAX.getInt(getActivity().getContentResolver(),
                Settings.XOPLAX.PIE_MIRROR_RIGHT, 1) == 1);

        float defaultAlpha;
        try{
            defaultAlpha = Settings.XOPLAX.getFloat(getActivity()
                     .getContentResolver(), Settings.XOPLAX.PIE_BACKGROUND_ALPHA);
        } catch (Exception e) {
            defaultAlpha = 0.3f;
            Settings.XOPLAX.putFloat(getActivity().getContentResolver(),
                Settings.XOPLAX.PIE_BACKGROUND_ALPHA, defaultAlpha);
        }
        mPieBackgroundAlpha.setInitValue((int) (defaultAlpha * 100));

        float controlSize;
        try{
            controlSize = Settings.XOPLAX.getFloat(getActivity()
                    .getContentResolver(), Settings.XOPLAX.PIE_SIZE);
        } catch (Exception e) {
            controlSize = PIE_CONTROL_SIZE_DEFAULT;
            Settings.XOPLAX.putFloat(getActivity().getContentResolver(),
                Settings.XOPLAX.PIE_SIZE, controlSize);
        }
