...
public class PowerSaverSettings extends SettingsPreferenceFragment implements
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        boolean powerSaverEnabled = Settings.XOPLAX.getInt(resolver,
                Settings.XOPLAX.POWER_SAVER_ENABLED, 0) != 0;
        ...
        if (mTogglesCPUProfile != null && !mPowerManager.hasPowerProfiles()) {
            ...
            mTogglesCPUProfile.setChecked(Settings.XOPLAX.getInt(resolver,
                    Settings.XOPLAX.POWER_SAVER_CPU_PROFILE, 0) != 0);
        }
        ...
        if (!Utils.fileExists(Processor.GOV_LIST_FILE) || !Utils.fileExists(Processor.GOV_FILE)
            ...
            mTogglesCPUGovernor.setChecked(Settings.XOPLAX.getInt(resolver,
                    Settings.XOPLAX.POWER_SAVER_CPU_GOVERNOR, 1) != 0);
        }
        ...
        mTogglesMobileData.setChecked(Settings.XOPLAX.getInt(resolver,
                Settings.XOPLAX.POWER_SAVER_MOBILE_DATA, 0) != 0);
        mTogglesGPS = (CheckBoxPreference) prefSet.findPreference(KEY_TOGGLES_GPS);
        mTogglesGPS.setChecked(Settings.XOPLAX.getInt(resolver,
                Settings.XOPLAX.POWER_SAVER_GPS, 0) != 0);
        mTogglesNotification = (CheckBoxPreference) prefSet
                .findPreference(KEY_TOGGLES_NOTIFICATION);
        mTogglesNotification.setChecked(Settings.XOPLAX.getInt(resolver,
                Settings.XOPLAX.POWER_SAVER_NOTIFICATION, 1) != 0);
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen, Preference preference) {
        if (preference == mTogglesCPUProfile) {
            ...
            mPowerManager.setPowerProfile(mTogglesCPUProfile.isChecked() ? pwrsvValue[0]
                    : pwrsvValue[1]);
            Settings.XOPLAX.putInt(resolver, Settings.XOPLAX.POWER_SAVER_CPU_PROFILE,
                    mTogglesCPUProfile.isChecked() ? 1 : 0);
        } else if (preference == mTogglesCPUGovernor) {
            Settings.XOPLAX.putInt(resolver, Settings.XOPLAX.POWER_SAVER_CPU_GOVERNOR,
                    mTogglesCPUGovernor.isChecked() ? 1 : 0);
        } else if (preference == mTogglesMobileData) {
            Settings.XOPLAX.putInt(resolver, Settings.XOPLAX.POWER_SAVER_MOBILE_DATA,
                    mTogglesMobileData.isChecked() ? 1 : 0);
        } else if (preference == mTogglesGPS) {
            Settings.XOPLAX.putInt(resolver, Settings.XOPLAX.POWER_SAVER_GPS,
                    mToggleGPS.isChecked() ? 1 : 0);
        }

        if (preference == mTogglesNotification) {
            Settings.XOPLAX.putInt(resolver, Settings.XOPLAX.POWER_SAVER_NOTIFICATION,
    ...
    public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
        Settings.XOPLAX.putInt(resolver, Settings.XOPLAX.POWER_SAVER_ENABLED, isChecked ? 1 : 0);
