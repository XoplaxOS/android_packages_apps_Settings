...
public class InputMethodsSettings extends SettingsPreferenceFragment implements
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mDisableFullscreenKeyboard.setChecked(Settings.XOPLAX.getInt(getContentResolver(),
                Settings.XOPLAX.DISABLE_FULLSCREEN_KEYBOARD, 0) == 1);
        ...
        mKeyboardRotationToggle.setChecked(Settings.XOPLAX.getInt(getContentResolver(),
                Settings.XOPLAX.KEYBOARD_ROTATION_TIMEOUT, 0) > 0);
        ...
        updateRotationTimeout(Settings.XOPLAX.getInt(
                getContentResolver(), Settings.XOPLAX.KEYBOARD_ROTATION_TIMEOUT,
                KEYBOARD_ROTATION_TIMEOUT_DEFAULT));

        mShowEnterKey = (CheckBoxPreference) findPreference(SHOW_ENTER_KEY);
        mShowEnterKey.setChecked(Settings.XOPLAX.getInt(getContentResolver(),
                Settings.XOPLAX.FORMAL_TEXT_INPUT, 0) == 1);
    ...
    public boolean onPreferenceChange(Preference preference, Object objValue) {
        if (preference == mDisableFullscreenKeyboard) {
            Settings.XOPLAX.putInt(getContentResolver(),
                    Settings.XOPLAX.DISABLE_FULLSCREEN_KEYBOARD,  (Boolean) objValue ? 1 : 0);
            return true;
        } else if (preference == mKeyboardRotationToggle) {
            boolean isAutoRotate = (Settings.XOPLAX.getInt(getContentResolver(),
                        Settings.XOPLAX.ACCELEROMETER_ROTATION, 0) == 1);
            if (isAutoRotate && mKeyboardRotationToggle.isChecked())
                mKeyboardRotationDialog();
            Settings.XOPLAX.putInt(getContentResolver(),
                    Settings.XOPLAX.KEYBOARD_ROTATION_TIMEOUT,
            ...
            Settings.XOPLAX.putInt(getContentResolver(),
                Settings.XOPLAX.FORMAL_TEXT_INPUT, (Boolean) objValue ? 1 : 0);
            return true;
        } else if (preference == mKeyboardRotationTimeout) {
            int timeout = Integer.parseInt((String) objValue);
            Settings.XOPLAX.putInt(getContentResolver(),
                    Settings.XOPLAX.KEYBOARD_ROTATION_TIMEOUT, timeout);
